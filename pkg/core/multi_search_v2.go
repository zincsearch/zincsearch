...
func MultiSearchV2(indexName string, query *meta.ZincQuery) (*meta.SearchResponse, error) {
	...
	for err == nil && next != nil {
		...
		if err != nil {
			log.Printf("core.MultiSearchV2: error accessing stored fields: %v", err)
			continue
		}
	...
	if err != nil {
		log.Printf("core.MultiSearchV2: error iterating results: %v", err)
	}
	...
	if err := parser.FormatResponse(resp, query, dmi.Aggregations()); err != nil {
		log.Printf("core.MultiSearchV2: error format response: %v", err)
	}
