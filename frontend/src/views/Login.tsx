...
export default function Login() {
  ...
  async function submit(event: FormEvent) {
    ...
    if (!id || !password) {
      setError('authErrors.credentialsRequired');
      return;
    }
    setBusy(true);
    setError('');
    try {
      ...
      setError(status === 401 ? 'authErrors.invalidCredentials' : 'authErrors.signInFailed');
    } finally {
      setBusy(false);
    }
  }
  return <main className="login-page">
    <form className="card login-card w-full max-w-sm space-y-5" onSubmit={submit} noValidate>
      ...
      <div><label className="block" htmlFor={passwordId}>{t('login.password')}</label><PasswordInput id={passwordId} className="mt-1 w-full" data-cy="login-password" autoComplete="current-password" value={password} onChange={e => setPassword(e.target.value)} required /></div>
      {error && <p className="error" role="alert">{t(error)}</p>}
      <button className="primary w-full" data-cy="login-sign-in" disabled={busy}>{busy ? t('authErrors.signingIn') : t('login.signIn')}</button>
    </form>
  </main>;
}
